# inspired by 
#   (1) https://github.com/Michedev/VAE_anomaly_detection/blob/master/model/VAE.py
#   (2) https://github.com/rasbt/stat453-deep-learning-ss21/blob/main/L17/1_VAE_mnist_sigmoid_mse.ipynb
...
class VAEAnomalyDetection(ABC):
    ...
    def __init__(
        ...
        self.encoder = self.create_encoder(input_size, latent_size)
        self.decoder = self.create_decoder(latent_size, input_size)
        return 
    
    @abstractmethod
    def create_encoder(self, input_size: int, latent_size: int) -> nn.Module:
        pass

    @abstractmethod
    def create_decoder(self, latent_size: int, output_size: int) -> nn.Module:
        pass
...
class VAEAnomalyTabular(VAEAnomalyDetection):
    ...
    def create_encoder(self, input_size:int, latent_size:int):
        """
    ...
    def create_decoder(self, latent_size, output_size):
        """
        ...
        return nn.Sequential(
            ...
            nn.Linear(200, 400),
            nn.ReLU(),
            nn.Linear(400, output_size * 2)  # times 2 because this is the concatenated vector of reconstructed mean and variance
        )
